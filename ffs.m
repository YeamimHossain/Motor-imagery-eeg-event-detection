function [in,history] = ffs(fun,varargin)
...
%   At this time, SEQUENTIALFS does not accept 'table' as input.
%
...
if isnumeric(cv)
    if isscalar(cv) && cv > 1 && round(cv) == cv %K-fold
...
if ~isempty(nfs)
    if ~isnumeric(nfs) || ~isscalar(nfs) || round(nfs) ~= nfs ||...
            nfs < 0 || nfs > p
        error(message('stats:sequentialfs:BadNfs'));
    else
        if any(keepin) && sum(keepin) > nfs
            error(message('stats:sequentialfs:InvalidNfsTooSmall'));
        end
        if  any(keepout) && p-sum(keepout) < nfs
            error(message('stats:sequentialfs:InvalidNfsTooLarge'));
        end
...
switch direction
    case 'forward'
        ...
        if options.Display > 1
            disp(getString(message('stats:sequentialfs:StartForward')));
            fprintf('%s',getString(message('stats:sequentialfs:InitialColumns',makeColText(keepin,p))));
            fprintf('%s',getString(message('stats:sequentialfs:ColumnsNotBeIncluded',makeColText(keepout,p))));
            if ~isempty(find(in,1)) || nullmodel
                fprintf('%s',getString(message('stats:sequentialfs:Step1InitialColumnsCriterion',sprintf('%g',history.Crit(end)))));
            end
        ...
        for j = 1:nStart
            ...
            if options.Display > 2 %iter
                fprintf('%s',getString(message('stats:sequentialfs:AddedColumnCriteri',...
                    length(history.Crit),nextOne,sprintf('%g',bestCrit)) ));
            end            
        end
        if options.Display > 1 % final or iter
            fprintf('%s',getString(message('stats:sequentialfs:FinalColumnsIncluded',makeColText(in,p))));
        end
    ...
    case 'backward'
        ...
        if options.Display >1
            fprintf(getString(message('stats:sequentialfs:StartBackward')));
            fprintf('%s',getString(message('stats:sequentialfs:InitialColumns',makeColText(~keepout,p))));
            fprintf('%s',getString(message('stats:sequentialfs:ColumnsMustBeIncluded',makeColText(keepin,p))));
            if ~isempty(find(in,1)) || nullmodel
               fprintf('%s',getString(message('stats:sequentialfs:Step1InitialColumnsCriterion',sprintf('%g',history.Crit(end)))));
            end         
        end
        ...
        for j = 1:nin_start
            ...
            history.Crit = [history.Crit, bestCrit];
            if options.Display > 2 %iter
                fprintf('%s',getString(message('stats:sequentialfs:RemovedColumnCrite',...
                    length(history.Crit),nextOne,sprintf('%g',bestCrit))));
            end
